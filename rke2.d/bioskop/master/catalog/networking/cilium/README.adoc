...
** `+cluster+`: 10.80.8.128/26 (addresses 129-190)
** `+lan+`: 192.168.1.64/27 (addresses 65-94)
** `+vip+`: 10.80.15.0/24 (addresses 1-254)
...
cd /var/lib/rke2lab/catalog/networking/cilium
...
# Should get IP from 10.80.8.129-190 range (cluster pool)
...
[source,bash]
----
kpt live destroy /var/lib/rke2lab/catalog/networking/cilium
...
[source,bash]
----
git checkout modules/nixos/rke2lab/make.d/cloud-config/cloud-config.master.cilium.yaml
# Redeploy master container
ssh lima-nerd-nixos "cd /var/lib/nixos/config/modules/nixos/rke2lab && source <(flox activate) && incus stop master --force && incus delete master && make NAME=master start"
----
...
├─────────────────────────────────────────────────────────────┤
│ IP Pools:                                                   │
│ • Cluster LB: 10.80.8.129-190 (62 IPs)                    │
│ • LAN LB: 192.168.1.65-94 (30 IPs)                        │
│ • VIP LB: 10.80.15.1-254 (254 IPs)                           │
├─────────────────────────────────────────────────────────────┤
│ L2 Announcements:                                           │
