...
public final class TektonPipelinesLayer extends Construct {
    ...
    private final PackageMetadataProfile packageProfile = new PackageMetadataProfile("cicd", "tekton-pipelines");

    public TektonPipelinesLayer(final Construct scope, final String id) {
        super(scope, id);

        createTektonOperatorNamespace();
        createReplicatedSecret("tekton-git-auth", "kubernetes.io/basic-auth", "kube-system/tekton-git-auth");
        createReplicatedSecret("tekton-docker-config", "kubernetes.io/dockerconfigjson",
        ...
        new ApiObject(
                ...
                ApiObjectProps.builder()
                        ...
                        .metadata(ApiObjectMetadata.builder()
                                ...
                                .annotations(packageProfile.packageAnnotations("|Namespace|default|tekton-operator"))
                                .build())
                        .build()
        );
    }

    private void createReplicatedSecret(final String name, final String type, final String replicateFrom) {
    ...
    private void createTektonConfig() {
        ApiObject config = new ApiObject(
                this,
                "tektonconfig-config",
                ApiObjectProps.builder()
        ...
        config.addJsonPatch(JsonPatch.add("/spec", Map.of(
                "profile", "all",
                "targetNamespace", "tekton-pipelines",
                "result", Map.of("disabled", true),
                "pruner", Map.of(
                        "resources", List.of("taskrun", "pipelinerun"),
                        "keep", 100,
                        "schedule", "0 8 * * *"
                )
        )));
