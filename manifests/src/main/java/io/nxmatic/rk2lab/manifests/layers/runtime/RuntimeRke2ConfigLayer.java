...
public final class RuntimeRke2ConfigLayer extends Construct {
    ...
    private final PackageMetadataProfile packageProfile = new PackageMetadataProfile("runtime", "rke2-config");

    public RuntimeRke2ConfigLayer(final Construct scope, final String id) {
        super(scope, id);

        createConfigMap("advertise-address.yaml", "Advertise address fragment", "|ConfigMap|default|rke2-advertise-address",
                Map.of("advertise-address", "10.80.0.10"));
        createConfigMap("cidrs.yaml", "Network CIDRs fragment", "|ConfigMap|default|rke2-cidrs",
                orderedMap(
        ...
        createConfigMap("cluster-init.yaml", "Cluster init flag (only true on first master)", "|ConfigMap|default|rke2-cluster-init",
                Map.of("cluster-init", true));
        createConfigMap("core.yaml", "Core RKE2 settings", "|ConfigMap|default|rke2-core",
                orderedMap(
        ...
        createConfigMap("etcd-metrics.yaml", "Etcd metrics fragment", "|ConfigMap|default|rke2-etcd-metrics",
                Map.of("etcd-expose-metrics", true));
        createConfigMap("etcd.yaml", "Etcd settings fragment", "|ConfigMap|default|rke2-etcd",
                orderedMap(
                        entry("with-node-id", false),
                        entry("node-name", "bioskop-master"),
                        entry("etcd-expose-metrics", false)
                ));
        createConfigMap("node-inetaddr.yaml", "Node IP fragment", "|ConfigMap|default|rke2-node-inetaddr",
                Map.of("node-ip", "10.80.0.10"));
        createConfigMap("node-labels.yaml", "Node labels fragment", "|ConfigMap|default|rke2-node-labels",
                Map.of("node-label", List.of(
                        "node.kubernetes.io/instance-name=master",
                        "node.kubernetes.io/instance-kind=server",
                        "flox.dev/enabled=true"
                )));
        createConfigMap("tls-san.yaml", "TLS SAN fragment", "|ConfigMap|default|rke2-tls-san",
                Map.of("tls-san", List.of(
                        "localhost",
                        "gateway",
                        "0.0.0.0",
                        "127.0.0.1",
        ...
        createConfigMap("token.yaml", "RKE2 token fragment", "|ConfigMap|default|rke2-token",
                        ...
                        .metadata(ApiObjectMetadata.builder()
                                ...
                                .annotations(packageProfile.packageAnnotations(
                                        ...
                                        Map.of(
                                                ...
                                                "description.kpt.dev", description
                                        )
                                ))
                                .build())
                        .build()
