...
installer::flox:install_environments() {
	...
	echo "ConfigMap mount contents:"
	find "${BUILD_ASSETS_DIR}" -type f | head -30

	# Discover all flox environment files in ConfigMap
	# Files are mounted at their mountPath: /.sh/build-assets/{category}/{name}/.flox/env/{file}
	# We need to iterate through the category/name directory structure
	# Note: flake.lock is NOT in ConfigMap (8.5MB each, exceeds ConfigMap size limits)
	# It will be generated by flox activate during installation

	local env_base="${BUILD_ASSETS_DIR}"
	for category_dir in "${env_base}"/*; do
		[[ -d "${category_dir}" ]] || continue
		category="${category_dir##*/}"

		# Skip non-environment directories
		[[ "${category}" == "debug-tools" ]] && continue
		[[ "${category}" == "nri-plugin."* ]] && continue
		...
		for name_dir in "${category_dir}"/*; do
			...
			# Install all files from this environment
			local flox_dir="${name_dir}/.flox"
			if [[ -d "${flox_dir}" ]]; then
				# Install .flox/env.json
				[[ -f "${flox_dir}/env.json" ]] &&
					install -D -m 0644 "${flox_dir}/env.json" "${HOST_SCRIPT_ROOT}/${category}/${name}/.flox/env.json"

				# Install .flox/env/* files
				if [[ -d "${flox_dir}/env" ]]; then
					for env_file in "${flox_dir}/env"/*; do
						[[ -f "${env_file}" ]] || continue
						file_name="${env_file##*/}"
						install -D -m 0644 "${env_file}" "${HOST_SCRIPT_ROOT}/${category}/${name}/.flox/env/${file_name}"
					done
				fi
			fi
		done
...
installer::host:flox:activate_environments() {
	...
	echo "This populates /nix/store with packages shared across all pods"

	local env_base="${FLOX_RUNTIME_ROOT}"
	local -a discovered_envs=()
	...
	for category_dir in "${env_base}"/*; do
		...
		# Skip non-environment directories
		[[ "${category}" =~ ^(bin|etc|log|debug-tools|nri-plugin|kube-cache)$ ]] && continue
	...
	# Pre-activate each environment to generate flake.lock and populate /nix/store
	for env_path in "${discovered_envs[@]}"; do
		local env_dir="${env_base}/${env_path}"

		echo "Activating ${env_path}..."
		if (cd "${env_dir}" && flox activate -- echo "  ✓ ${env_path} environment resolved"); then
