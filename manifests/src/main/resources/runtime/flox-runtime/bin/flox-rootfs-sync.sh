...
: "${FLOX_RUNTIME_SYNC_NAMESPACE:?FLOX_RUNTIME_SYNC_NAMESPACE is required}"
: "${FLOX_RUNTIME_SYNC_ID:?FLOX_RUNTIME_SYNC_ID is required}"

SYNC_LOG="${FLOX_RUNTIME_SYNC_LOG:-/var/log/rke2lab/flox-rootfs-sync.log}"
BUNDLE_ROOT="${FLOX_RUNTIME_BUNDLE_ROOT:-/run/k3s/containerd/io.containerd.runtime.v2.task}"
WAIT_TIMEOUT_SECS="${FLOX_RUNTIME_SYNC_TIMEOUT_SECS:-15}"
POLL_INTERVAL_SECS="${FLOX_RUNTIME_SYNC_POLL_INTERVAL_SECS:-0.1}"
...
main() {
	...
	bundle_dir="${BUNDLE_ROOT%/}/${FLOX_RUNTIME_SYNC_NAMESPACE}/${FLOX_RUNTIME_SYNC_ID}"
	rootfs_dir="${bundle_dir}/rootfs"
	started_at="$(date +%s)"

	log "watching bundle namespace=${FLOX_RUNTIME_SYNC_NAMESPACE} id=${FLOX_RUNTIME_SYNC_ID} bundle=${bundle_dir}"
