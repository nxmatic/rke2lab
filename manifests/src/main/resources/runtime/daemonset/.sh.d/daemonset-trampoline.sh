...
# - DAEMONSET_HOST_SCRIPT_ROOT=/srv/host/... (required for pod/guest re-exec)
...
daemonset::trampoline:exec_on_host() {
	...
	pod)
		...
		exec nsenter --target 1 --mount --uts --ipc --net --pid -- env \
			...
			DAEMONSET_HOST_SCRIPT_ROOT="${DAEMONSET_HOST_SCRIPT_ROOT}" \
			DAEMONSET_HOST_SCRIPT_BIN="${host_script_bin}" \
	...
	guest)
		...
		remote_command+=" $(printf '%q' "DAEMONSET_HOST_SCRIPT_ROOT=${DAEMONSET_HOST_SCRIPT_ROOT}")"
		remote_command+=" $(printf '%q' "DAEMONSET_HOST_SCRIPT_BIN=${host_script_bin}")"
