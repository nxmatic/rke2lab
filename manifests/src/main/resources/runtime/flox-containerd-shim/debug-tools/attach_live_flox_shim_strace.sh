...
usage() {
	cat <<'EOF'
Usage:
  attach_live_flox_shim_strace.sh --shim-id <sandbox-id> [--log-root <dir>] [--output-dir <dir>]
  attach_live_flox_shim_strace.sh --pid <shim-pid> [--log-root <dir>] [--output-dir <dir>]

Description:
  Finds the long-lived flox shim daemon for a sandbox and attaches strace to all
...
printf 'Attaching to live flox shim daemon\n'
printf '  pid: %s\n' "${TARGET_PID}"
