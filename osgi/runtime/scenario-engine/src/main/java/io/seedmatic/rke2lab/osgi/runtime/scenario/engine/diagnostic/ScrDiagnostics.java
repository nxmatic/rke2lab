package io.seedmatic.rke2lab.osgi.runtime.scenario.engine.diagnostic;
...
public final class ScrDiagnostics {
  ...
  /**
   * Bind to the {@link ServiceComponentRuntime} felix.scr publishes in {@code context}, or {@link
   * Optional#empty()} if SCR is not present (the framework was booted without {@code withScr()}).
   * The absence is a real runtime condition surfaced at its source — the boundary lookup that
   * returns no reference — so the caller decides how to read a world without SCR rather than
   * dereferencing a null.
   */
  public static Optional<ScrDiagnostics> of(BundleContext context) {
    final ServiceReference<ServiceComponentRuntime> reference =
        context.getServiceReference(ServiceComponentRuntime.class);
    if (reference == null) {
      return Optional.empty();
    }
    return Optional.of(new ScrDiagnostics(context.getService(reference)));
  }
      ...
      for (ComponentConfigurationDTO configuration :
        ...
        for (UnsatisfiedReferenceDTO unsatisfied : configuration.unsatisfiedReferences) {
          report
              ...
              .append(" target=")
              .append(targetOf(description, unsatisfied.name).orElse("(none)"))
              .append('\n');
        }
      }
    }
  ...
  /**
   * The declared target filter of a named reference, or {@link Optional#empty()} if it states none
   * — the SCR DTO's {@code target} is null when no filter is declared, wrapped at that frontier so
   * {@link #report()} resolves the empty case.
   */
  private static Optional<String> targetOf(
      ComponentDescriptionDTO description, String referenceName) {
    for (ReferenceDTO reference : description.references) {
      if (reference.name.equals(referenceName)) {
        return Optional.ofNullable(reference.target);
      }
    }
    return Optional.empty();
  }
