 ...
 * scenario run, the {@link ScenarioOutcome} the {@link ScenarioOutcomeExtension} seeded into the
 * {@code sessionStore} (the launcher session's own store, the OUTBOUND channel's read side).
 *
 ...
 * ScenarioOutcome} back from the session store (via {@link ScenarioOutcomeSeed#read}) and returns
