 ...
 * {@code io.nxmatic.rke2lab.manifests.bdd} so the passenger it enumerates runs on the host bundle's
 * loader — the loader the scenario resolves its collaborators against, so the mock RunGate the
 * passenger registers is the same Class the scenario reads.
 *
