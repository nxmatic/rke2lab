...
# VENDORED VERBATIM from k3s-io/k3s: contrib/util/generate-custom-ca-certs.sh
# Upstream: https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh
# License: Apache-2.0 (SUSE Rancher / k3s authors). DO NOT EDIT — this is the exact upstream tool
# RKE2 documents for bring-your-own-CA. cluster-ca-ceremony.sh (beside this file) drives it with
# PRODUCT=rke2 and a staging DATA_DIR, feeding it our own root (root-ca.pem/root-ca.key derived from
# the ndh mammoth-skate-tls authority). See docs/architecture/cluster-api/deterministic-cluster-access.adoc.
