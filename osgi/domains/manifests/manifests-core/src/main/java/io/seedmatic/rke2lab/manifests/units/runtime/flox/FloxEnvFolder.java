 ...
 * flox.dev/environment.<container>} pod annotation's {@code <folder>/} prefix that the flox NRI
 * plugin keys on. A typed vocabulary prevents the folder ↔ annotation-prefix mismatches that would
 * silently misroute a workload's env (the plugin would readlink a GC-root the controller never
 * provisioned).
 *
