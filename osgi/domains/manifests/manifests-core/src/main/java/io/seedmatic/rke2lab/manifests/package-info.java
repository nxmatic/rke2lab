 ...
 *
 * <ul>
 *   <li>{@link io.seedmatic.rke2lab.manifests.ManifestsUnit} — the atomic build block; a CDK8s
 *       {@code Construct} that synthesizes one logical group of K8s resources and (optionally) the
 *       systemd installer service for them.
 *   <li>{@link io.seedmatic.rke2lab.manifests.AbstractManifestsUnit} — base class wiring a unit
 *       into the CDK8s construct tree.
 *   <li>{@link io.seedmatic.rke2lab.manifests.ManifestsDomain} — groups related units and declares
 *       cross-domain dependency ordering.
 *   <li>{@link io.seedmatic.rke2lab.manifests.DefaultManifestSynthesisService} — the orchestrator:
 *       creates the {@code App}, the K8s {@code Chart} and the {@code SystemdChart}, then writes
 *       both output trees.
 *   <li>{@link io.seedmatic.rke2lab.manifests.ManifestSynthesisContext} — ThreadLocal runtime
 *       configuration injected into units for the duration of one synthesis.
 *   <li>{@link io.seedmatic.rke2lab.manifests.contract.ManifestDomainCatalog} — single source of
 ...
 *
 * <ul>
 *   <li>{@code domain} — the {@link io.seedmatic.rke2lab.manifests.ManifestsDomainRegistrar}
