...
/**
 * The edge's private failure flow while shelling {@code nix} / {@code ssh}: thrown deep in the
 * process plumbing (a missing binary, a non-zero exit, an ssh failure) and caught once at the
 * {@link CultivatingNixosImageBuilder#build} boundary, where it becomes the seam's human summary.
...
final class ImageBuildException extends RuntimeException {
   ...
   * Throwable#getMessage} is {@code @Nullable}; the {@link CultivatingNixosImageBuilder#build}
