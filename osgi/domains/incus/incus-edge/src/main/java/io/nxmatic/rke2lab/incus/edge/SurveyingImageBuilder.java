 ...
 * {@code distrobuilder}, opens no {@code ssh}: {@link #build} returns {@link Optional#empty()} (the
 * honest plan — the build WOULD run cleanly; a survey cannot claim the artifacts exist, and the
 * step renders PENDING, so this is a plan, never a fabricated success). The single honest thing a
 * survey CAN assert is the recipe {@link #recipeDigest() digest} — pure over the bundle resources,
 * so it is IDENTICAL to the cultivating builder's; the host's image-cache key must not move between
 * the modes.
 *
 ...
 * CultivatingDistrobuilderImageBuilder}, builds for real.
 */
