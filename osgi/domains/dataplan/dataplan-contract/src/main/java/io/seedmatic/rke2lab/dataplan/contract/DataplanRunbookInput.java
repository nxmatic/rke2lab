...
/**
 * The wire contract for the dataplan {@code runbook} trigger — the activation payload a sower
 * supplies to play the dataplan layout-export scion. The INPUT twin of a reaped wire-record: the
 ...
 * is the plot the scion writes {@code dataplan.json} into, which only the host knows (it creates
 ...
 * temp dir; absence is an empty {@link Optional}, never a blank string. The layout itself is fully
 * declared in-container ({@link DataplanLayout#canonical} is a pure factory), so this input carries
 * no facet — where to write is the only host-held fact.
 */
@SeedContract("runbook")
public record DataplanRunbookInput(
