 ...
 * history and yields one opaque {@code visit} {@link Document} per readable entry, WITHOUT
 * interpreting its medical content. Each Document's payload carries the entry's {@code version} +
 ...
 * seam — only {@link Patient} (the read key) and {@link Document} (the opaque blob).
 */
public interface MedicalRecordJournal {

  /**
   * The patient's own stack timeline, oldest first, one {@code visit} {@link Document} per readable
   * history entry. An absent or empty timeline yields an empty list (a legitimate nothing-here); a
   * present-but-unreadable history is the implementation's concern and degrades to the readable
   * prefix rather than throwing into the diagnosis path.
   */
  List<Document> historyOf(Patient patient);
