 ...
 * SeedEnvelope}. Unkeyed: there is exactly one ledger stack ({@code intervention-ledger/dev}), so
 * there is nothing to key the read by.
 */
public interface InterventionJournal {

  /**
   * The ledger's history, oldest first, one {@code intervention} {@link SeedEnvelope} per entry. An
