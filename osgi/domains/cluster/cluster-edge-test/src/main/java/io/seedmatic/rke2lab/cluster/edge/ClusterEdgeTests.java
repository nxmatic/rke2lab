 ...
 * {@code io.seedmatic.rke2lab.cluster.edge} so the passenger it enumerates runs on the host
 * bundle's loader — the realm the edge's SCR-published ClusterReadinessContact lives in, so the
 * passenger resolves and calls it TYPED without cluster.contract being system-exported.
 */
