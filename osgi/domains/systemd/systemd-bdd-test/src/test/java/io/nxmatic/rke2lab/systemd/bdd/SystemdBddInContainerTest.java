 ...
 * (via {@link JGivenTestkit#felix()}, since the scenario is a jGiven ScenarioTest) — installs the
...
class SystemdBddInContainerTest {
  ...
  private static final String RUNNER_FQN = "io.nxmatic.rke2lab.systemd.bdd.SystemdBddTests";
  ...
  Stream<DynamicTest> scionTests() throws Exception {
    // Install the fixture + its host (systemd-bdd), located through the fragment's Fragment-Host —
    // no
    ...
    final Bundle host = felix.installFixtureWithHost(SYSTEMD_BDD_FIXTURE).host();
    ...
    if (!felix.resolve(toResolve)) {
      fail("systemd-bdd (with its -test fragment) must resolve");
    }
    host.start();
    ...
    return results.stream().map(SystemdBddInContainerTest::toDynamicTest);
  }
