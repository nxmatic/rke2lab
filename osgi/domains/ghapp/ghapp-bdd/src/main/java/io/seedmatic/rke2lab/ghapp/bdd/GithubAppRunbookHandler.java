...
/**
 * The ghapp runbook handler — the OSGi-side grower behind the broker's one host→scion door. Extends
...
public final class GithubAppRunbookHandler extends GenericRunbookHandler {
  ...
  private static final RunbookCoordinate COORDINATE = new RunbookCoordinate("ghapp");
  ...
  public Class<? extends ScenarioPlayer.Playable> scenarioClass() {
    return GithubAppScenario.class;
  }

  @Override
  public Consumer<NamespacedHierarchicalStore<Namespace>> seedFrom(SeedEnvelope trigger) {
    // No input: the scion reads the manifest resource and drives its edges in-container.
    return store -> {};
  }
}
