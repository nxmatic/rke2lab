 ...
 * — no scion reads the cellar. Both ends touch the {@link OpaqueCellar}; the four crossings between
 * them only sow and graft.
 *
 * <p>The {@link OpaqueCellar} is handed in, not resolved here: seed-bdd is foundation and cannot
 * name a realisation ({@code PulumiCellar} is host, a git-backed cellar is another exec's). The
 * exec injects the one its gardening version uses through the {@link Hidden} wiring step, the way
 * every other stage receives its collaborators. seed-bdd depends only on the neutral {@link
 * OpaqueCellar} seam.
 */
public class CellarStage extends Stage<CellarStage> {

  @ScenarioState private OpaqueCellar cellar;
  ...
  /**
   * Hand in the conservation collaborators: the realised {@link OpaqueCellar} and the {@link
   * Parcel}.
   */
  @Hidden
  public CellarStage conserving(OpaqueCellar cellar, Parcel parcel) {
    this.cellar = cellar;
    this.parcel = parcel;
    return self();
  }
