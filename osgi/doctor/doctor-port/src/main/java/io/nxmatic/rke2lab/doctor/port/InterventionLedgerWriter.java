 ...
 * io.nxmatic.rke2lab.pulumi.automation.StackHandle}. Implementations persist an {@link
 * Intervention} to a stack. The interface exists so tests can fake it without running Pulumi, and
 * so the operator command and the drift specialist depend on the contract, not on Pulumi internals.
 */
