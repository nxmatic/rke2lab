...
public final class MedicalRecordReconstructionException extends Exception {
   ...
   * lacks. The leaf {@link SnapshotException} is the cause, so the human reads identity here and,
   * via {@code getCause().location()}, the offending source — without reproducing the failure.
   */
  public static final class EntryFailure extends Exception {
    ...
    public EntryFailure(SnapshotEntry entry, SnapshotException cause) {
      super(
