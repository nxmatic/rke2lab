...
- When in a git repository and a task is about to **mutate** files (edit, commit, or any write), create a dedicated external worktree first. Pure read-only / investigation questions stay in the current checkout; don't spin one up for them. The startup recipe (run from `<repo>.d/main`):
  ...
  5. **Generate the `.code-workspace`** as a *sibling of the worktree*, inside the namespace dir: `<repo>.d/<namespace>/<slug>.code-workspace` (e.g. `chore/claude-memory-cascade.code-workspace` next to the `chore/claude-memory-cascade/` checkout). Name it with the bare leaf slug (the namespace is already carried by the parent dir); set the folder `path` *relative* to that sibling (just `<slug>`), and the folder `name` to `<namespace>/<slug>` so the VSCode tab stays distinct. Mirror the `settings` (flox JavaSE-25 runtime) from an existing sibling `.code-workspace`. These files live in the non-git `<repo>.d/` container, so they are not committed.
