...
*How `SPEC_COVERAGE` behaves during the migration (it is a code-first ratchet, NOT a violation counter):* the gate requires every type a bundle EXPORTS to be named in some `docs/` spec. The `pipeline-port` seam that once exported the fluent-grammar types has been **dissolved** — its `Phase`/`PhaseRunner` engine relocated into `manifests-core`'s un-exported internal synthesis, so those exports (and their spec-coverage obligation) are gone. What survives on the former model is the *host's own* `during/then` chains (`ClusterSeedTopic.seedCluster()`), the last to migrate; while any bundle still exports a type, the gate DEMANDS it stay documented — you cannot drop it from the specs before the CODE stops exporting it. Migration progress is tracked in the migration plan (N pipelines still on Topic/State), not by a gate count.
