...
6. *route to a remediator (transverse)* — hand the error off to a remediation handler instead of
   ...
   *dogfooding*: an error IS a symptom, handling-it-at-the-right-level IS the consultation, calling a
   remediator IS the prescription ([[doctor-remediation-model]]). The layered error contract is itself
...
([[doctor-remediation-model]]), NOT code for the current offline-dump tasks — the dump just fails its own
...
Relates to [[medical-record-query-api-state]]; consistent with the project's anti-silent-failure stance
