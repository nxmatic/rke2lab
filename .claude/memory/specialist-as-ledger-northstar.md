...
repo for neighbours", units = git trees, OSGi-resolution + p2p) and [[serviceloader-specialist-spi]]
...
treatment. This IS the "agenda loop" deferred in [[referral-roundtrip-state]]. The drift trigger falls
...
abstraction (extract only if a 2nd appears). See [[intervention-provenance-state]] for the chantier.
