...
- `osgi/manifests/manifests-cdk8s` (BSN `io.nxmatic.rke2lab.manifests.cdk8s`) embeds the flat closure
...
io.nxmatic.rke2lab.manifests.cdk8s` (attaches to the manifests carrier). Module name reads
...
package OWNED by the contributing domain. Renamed `io.nxmatic.rke2lab.cdk8s.systemd` →
`io.nxmatic.rke2lab.systemd.cdk8s` so the leaf sits under the `systemd` root (alongside
