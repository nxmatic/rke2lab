...
- [Post-clean-split cleanup handoff](post-cleansplit-cleanup-handoff.md) — **★ DO FIRST in the next main-window session (after reloading rke2lab.code-workspace).** Punch-list of known-safe leftovers THIS session couldn't delete (it ran on the old `.claude/hub` config-home): old hub-path `projects/` runtime (main + design), stale merged branch `refactor/config`. Delete the note when done. See [[claude-memory-cascade-state]].

- [Claude memory cascade state](claude-memory-cascade-state.md) — **★ THE subject of this branch (chore/claude-memory-cascade).** Three-tier cascade worktree→hub→home; brainstorm FROZEN 2026-06-18, spec written (`wip/specs/2026-06-18-claude-memory-three-tier-cascade-design.adoc`), next = writing-plans. Holds the 5 hard-won facts about Claude's file model (slug encodes `/`+`.`; **clean-split: `.code-workspace` sets CLAUDE_CONFIG_DIR=toplevel/.claude, superseding the old wrapper→`.claude/hub`**; slug=toplevel NOT git-dir; conversations ephemeral + `closed -` sidebar noise; **bridging is human — never graft a transcript across worktrees**). See [[hub:claude-auto-memory-mechanics]] [[hub:external-worktree-operating-model-state]].
