 ...
 * the operator targeted, which fails with the given {@link SymptomKind}. Lets a preview render the
...
public final class SimulatedClusterReadinessProbe {
  ...
  public static ClusterReadinessProbe failingAt(
      ClusterReadinessPhase failingPhase, SymptomKind symptom) {
    return (config, phase) ->
        phase == failingPhase
            ? ObservationView.failed(
                symptom,
                phase.label() + " failed (simulated incident: " + symptom.slug() + ")",
