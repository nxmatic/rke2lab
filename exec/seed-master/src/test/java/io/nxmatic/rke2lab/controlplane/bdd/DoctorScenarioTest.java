...
class DoctorScenarioTest
  ...
  void connection_refused_is_prescribed_a_unit_restart() {
    given()
        .a_doctor_staffed_with_the_dbus_specialist()
        .and()
    ...
    then()
        ...
        .and()
        .the_prescription_targets_unit(DbusTcpSpecialist.ADAPTER_UNIT)
        .and()
  ...
  void a_symptom_no_specialist_covers_yields_an_empty_but_named_plan() {
    // TIMEOUT routes to the NETWORK domain, which the dbus specialist does not cover.
    given().a_doctor_staffed_with_the_dbus_specialist().and().a_failure_presenting(Symptom.TIMEOUT);
    when().the_doctor_is_consulted();
  ...
  private void recognized_but_untreated(Symptom symptom) {
    given().a_doctor_staffed_with_the_dbus_specialist().and().a_failure_presenting(symptom);
    when().the_doctor_is_consulted();
    then().no_treatment_is_offered().and().the_plan_still_names_the_symptom();
  }
}
