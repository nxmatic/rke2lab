 ...
 *
 * <p>Derived from the same recipe model used in rke2lab network make rules
 * (host/lan/lb/node/vip).</p>
 */
public record ClusterNetworkBlueprint(
    ...
    private static ClusterNetworkBlueprint derive(String clusterName, String nodeName) {
        ...
        final String wanDhcpRange = "10.80." + hostThirdOctet + ".2-10.80." + hostThirdOctet
                + ".9,10.80." + hostThirdOctet + ".31-10.80." + (hostThirdOctet + 7) + ".254";

        final MacAddress wanHostMacaddr = MacAddress.parse(String.format("52:54:00:%02x:%02x:%02x", clusterId, nodeType.numericCode(), nodeId));
        final MacAddress lanHostMacaddr = MacAddress.parse(String.format("10:66:6a:4c:%02x:%02x", clusterId, nodeId));
        final MacAddress lanBridgeMacaddr = MacAddress.parse(String.format("02:00:00:bb:%02x:%02x", clusterId, nodeId));
    ...
    public String ref() {
        return "cluster:"
                ...
                + ",host:" + host.superNetworkCidr()
                + ",cluster:" + host.clusterCidr()
                + ",node:" + nodeNetwork.nodeCidr()
                + ",vip:" + vip.vipCidr()
                + ",lb:" + loadBalancer.lbCidr()
                + ",lan-node:" + lan.nodeCidr()
                + ",lan-lb:" + lan.lbCidr();
    }
    ...
    public record HostPlan(Cidr superNetworkCidr, Cidr clusterCidr, InetAddress clusterGatewayInetaddr) {
    }

    public record NodeNetworkPlan(Cidr nodeCidr, InetAddress nodeGatewayInetaddr, InetAddress nodeHostInetaddr) {
    }

    public record VipPlan(Cidr vipCidr, InetAddress vipGatewayInetaddr, InetAddress vipHostInetaddr) {
    }
