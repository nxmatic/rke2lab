...
4. **Identity Secret extraction in `IncusResourceBootstrap.HostStage`** (Stage A side): read operator's `~/.config/incus/{client.crt,client.key,servercerts/<remote>.crt}` and emit `Secret bioskop-incus-identity` in `capn-system` matching CAPN's `LXCCluster.spec.secretRef` schema (`server`, `server-crt`, `client-crt`, `client-key`).
