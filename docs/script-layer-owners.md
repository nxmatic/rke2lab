...
- Do not move unrelated scripts under `runtime/flox-runtime`.
...
- Deferred cleanup note: some inline mesh/runtime ConfigMaps (for example `mesh/headplane` Flox env payloads) are candidates to adopt the same deterministic serialized-asset/archive pattern.
