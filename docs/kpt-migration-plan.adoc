...
upstream:
  type: git
  git:
    repo: https://github.com/nxmatic/rke2lab
    directory: modules/nixos/rke2lab/catalog/mesh/headscale
    ref: develop
pipeline:
  mutators:
...
This migration plan provides a structured approach to introducing kpt into the rke2lab deployment system. The phased migration strategy reduces risk by starting with non-critical components and gradually moving to core infrastructure.
