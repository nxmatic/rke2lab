...
* *ENV delivery moved to runtime — as a CRD (2026-08-27).* The plugin stays BAKED
  ...
  `FloxEnv` CR reconciled by OUR *flox-envs controller* (a node-agent DaemonSet from
  its own repo `github:seedmatic/flox-envs-controller`) onto the node's host `/nix/store`.
  The plugin overlays the host
  `/nix` and resolves a pod's `flox.dev/environment` annotation *as a reference to a
...
flowchart TB
    ...
    Nix[(host /nix/store — plugin overlays this<br/>TARGET: realised by the flox-envs controller<br/>INTERIM: baked by nixos/flox-runtime.nix)]
    Catalog[(FloxEnv CRs<br/>referenced by the pod annotation<br/>ns/name)]

    ForkFlake -->|flake input, nix build| Bake
    Bake -->|launches| NRI
    User -->|Deploy pods with<br/>flox.dev/environment=ns/name| Containerd
...
`/nix/store`; in the target that store is populated by OUR flox-envs controller from
...
| `flox.dev/environment.<container>` | Yes | Reference to a `FloxEnv` CR: `[<namespace>/]<name>` (bare name = the pod's namespace; shared envs explicit, e.g. `flox-system/kdns`) | `flox-system/kdns`
...
* *CR-resolved (target):* the plugin looks up the `FloxEnv` CR `(namespace, name)`,
  ...
  host `/nix/store` path — which OUR flox-envs controller has realised + GC-rooted.
