...
* *ENV delivery moved to runtime — as a CRD (2026-08-27).* The plugin stays BAKED
  (foundation), but the flox *envs* it injects are no longer baked: each is a
  `FloxEnv` CR reconciled by OUR *flox controller* (a node-agent DaemonSet from
  its own repo `github:seedmatic/flox-controller`) onto the node's host `/nix/store`.
  The plugin overlays the host
  `/nix` and resolves a pod's `flox.seedmatic.io/environment` annotation *as a reference to a
  FloxEnv CR*. No PVC. Where a diagram/section shows the env baked, read it as the LIVE
  interim; the target populator is the controller (design doc §Runtime env delivery).
...
flowchart TB
    ...
    Nix[(host /nix/store — plugin overlays this<br/>TARGET: realised by the flox controller<br/>INTERIM: baked by nixos/flox-runtime.nix)]
    Catalog[(FloxEnv CRs<br/>referenced by the pod annotation<br/>ns/name)]

    ForkFlake -->|flake input, nix build| Bake
    Bake -->|launches| NRI
    User -->|Deploy pods with<br/>flox.seedmatic.io/environment=ns/name| Containerd
    Containerd -->|NRI API: Events| NRI
    NRI -->|resolve annotation → FloxEnv CR| Catalog
    NRI -->|overlay host /nix| Nix
...
round-trip, no runtime installer. The env *store* it overlays is the node's host
`/nix/store`; in the target that store is populated by OUR flox controller from
`FloxEnv` CRs (design doc §Runtime env delivery), and the plugin resolves the pod
annotation as a reference to a FloxEnv CR; the baked env is the LIVE interim.
...
flowchart TB
    ...
    subgraph NRI["NRI Plugin Process"]
        ...
        Parser[Annotation Parser<br/>flox.seedmatic.io/*]
    ...
    Event --> Handler
    Handler --> Parser
    Parser -->|Extract flox.seedmatic.io/environment| Resolver
...
sequenceDiagram
    ...
    Note over Containerd: Parse annotations: flox.seedmatic.io/environment=networking/kdns

    Containerd->>NRI: CreateContainer(pod, container)

    Note over NRI: Check flox.seedmatic.io/environment annotation
...
. *Annotation Detection*: NRI plugin checks `flox.seedmatic.io/environment`
...
| `flox.seedmatic.io/environment.<container>` | Yes | Reference to a `FloxEnv` CR: `[<namespace>/]<name>` (bare name = the pod's namespace; shared envs explicit, e.g. `flox-system/kdns`) | `flox-system/kdns`
| `flox.seedmatic.io/home` | No | Override HOME directory | `/home/app`
| `flox.seedmatic.io/uid` | No | Container UID (for chown) | `1000`
| `flox.seedmatic.io/gid` | No | Container GID (for chown) | `1000`
| `flox.seedmatic.io/debug` | No | Enable debug mode | `true`
| `flox.seedmatic.io/debug-port` | No | Delve debugger port | `2345`
|===

=== Environment Path Resolution

* Format: `[<namespace>/]<name>` — a reference to a `FloxEnv` CR. Bare `<name>`
  defaults to the pod's namespace (k8s-conventional); shared envs are referenced
  explicitly, e.g. `flox-system/kdns`. (The legacy flat `category/name` string is
  superseded.)
* *CR-resolved (target):* the plugin looks up the `FloxEnv` CR `(namespace, name)`,
  validates it is realised on this node (via `status`), and resolves to the env's
  host `/nix/store` path — which OUR flox controller has realised + GC-rooted.
* *Interim (LIVE):* the env is baked into the node `/nix/store` by
  `nixos/flox-runtime.nix` and resolved by `readlink`-ing the GC-root
  `/nix/var/nix/gcroots/flox-runtime/env/<category>/<name>`.
* *Legacy (pre-substrate):* mapped to
  `/srv/host/k8s-daemonset.d/runtime/flox-runtime/{category}/{name}` by the DaemonSet
  installer — dissolved by the substrate.
...
* link:atlas/nixos-substrate.adoc[Atlas — NixOS substrate] - the before/after figure for the baked-envs → FloxEnv-CRD env-delivery shift
