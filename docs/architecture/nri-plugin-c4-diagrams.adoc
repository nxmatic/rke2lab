...
`nixos/containerd.nix`, containerd launches it, and it resolves envs from the
local rke2lab catalog + the shared `/nix/store` — no FloxHub round-trip, no runtime
installer.
...
* *Foundation (store-resolved, current):* `{category}/{name}` → readlink the
  GC-root `/nix/var/nix/gcroots/flox-runtime/env/{category}/{name}` → the
  `/nix/store/...` env path. The env catalog lives in rke2lab's `runtime/flox`
  flake; the shim (fork) never holds it.
