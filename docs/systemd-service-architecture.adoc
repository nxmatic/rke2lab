...
multi-user.target
    ↑
    ├─ rke2lab-flox-builds-complete.target
    │   └─ After: rke2lab-flox-build.service
    │
...
*Target*: `rke2lab-flox-builds-complete.target`

* All Nix packages built from `flox-builds.yaml`
...
* Runs readiness check as `ExecStartPost` (`rke2lab-cilium-ready.sh`)
...
* Runs readiness check as `ExecStartPost` (`rke2lab-openebs-ready.sh`)
...
local-fs.target
    ↓
rke2lab-flox-build.service (builds all Nix packages)
    ↓
┌─────────────────────────────────────────────┐
│ rke2lab-flox-builds-complete.target     │  ← Guarantees packages available
└─────────────────────────────────────────────┘
----
...
[Service]
Type=oneshot
ExecStart=/srv/host/scripts.d/rke2lab-manifests-install.sh mycustom
ExecStartPost=/srv/host/scripts.d/rke2lab-layer-ready.sh mycustom
...
Requires=rke2lab-storage-ready.service
After=rke2lab-flox-build.service
Requires=rke2lab-flox-build.service
----
...
* Flox build service: link:../make.d/incus/systemd/rke2lab-flox-build.service[rke2lab-flox-build.service]
* Flox builds target: link:../make.d/incus/systemd/rke2lab-flox-builds-complete.target[rke2lab-flox-builds-complete.target]
* Mesh manifests service: link:../make.d/incus/systemd/rke2lab-mesh-manifests.service[rke2lab-mesh-manifests.service]
* Build descriptor: link:../rke2.d/bioskop/master/flox-builds.yaml[flox-builds.yaml]
