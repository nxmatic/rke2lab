...
| `runtime/flox-runtime`
| `runtime/flox-runtime`
| `manifests/src/main/resources/runtime/flox-runtime/runtime-installer.sh`

| `install.sh`
| `networking/envoy-gateway`
| `networking/envoy-gateway`
| `manifests/src/main/java/io/seedmatic/rke2lab/manifests/layers/networking/EnvoyGatewayLayer.java` (inline ConfigMap data)

| `kdns-dlv.sh`
| `networking/kdns`
| `networking/kdns`
| `manifests/src/main/java/io/seedmatic/rke2lab/manifests/layers/networking/KdnsLayer.java` (inline ConfigMap data)

| `agent-sync.sh`
| `mesh/headplane`
| `mesh/headplane`
| `manifests/src/main/java/io/seedmatic/rke2lab/manifests/layers/mesh/HeadplaneLayer.java` (inline ConfigMap data)

| `config-init.sh`
| `mesh/headscale`
| `mesh/headscale`
| `manifests/src/main/java/io/seedmatic/rke2lab/manifests/layers/mesh/HeadscaleLayer.java` (inline ConfigMap data)
...
| `host/systemd-scripts`
| `manifests` resources (materialized by `controlplane/incus`)
| `manifests.d/host/systemd-scripts`
| source path `manifests/src/main/resources/host/systemd-scripts`, mounted into node at `/srv/host/systemd-scripts.d`

| `host/systemd-units`
| `manifests` resources (materialized by `controlplane/incus`)
| `manifests.d/host/systemd-units`
| source path `manifests/src/main/resources/host/systemd-units`, stowed by `rke2lab-systemd-link.sh` from `/srv/host/systemd-units.d`
|===
...
* Owner code path: `controlplane/src/main/java/io/seedmatic/rke2lab/controlplane/incus/IncusResourceBootstrap.java`
